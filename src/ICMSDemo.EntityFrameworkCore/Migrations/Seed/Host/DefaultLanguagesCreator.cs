...
namespace ICMSDemo.Migrations.Seed.Host
{
    public class DefaultLanguagesCreator
    {
        public static List<ApplicationLanguage> InitialLanguages => GetInitialLanguages();

        private readonly ICMSDemoDbContext _context;

        private static List<ApplicationLanguage> GetInitialLanguages()
        {
            var tenantId = ICMSDemoConsts.MultiTenancyEnabled ? null : (int?)1;
        ...
        public DefaultLanguagesCreator(ICMSDemoDbContext context)
        {
            _context = context;
        }
